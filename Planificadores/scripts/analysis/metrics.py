...
def calcular_metricas_preemptive(df, algorithm, input_type):
    metrics = {
        'algorithm': algorithm,
    ...
    df_first_execution = df_sorted.copy().drop_duplicates(
        subset='ID Proceso', keep='first')
    df_last_execution = df_sorted.drop_duplicates(
        subset='ID Proceso', keep='last')
    ...
    metrics['tiempo_espera_promedio'] = \
        df_first_execution['Tiempo de Espera'].mean()

    metrics['var_tiempo_espera'] = \
        df_first_execution['Tiempo de Espera'].std()
    ...
    tiempo_total_observado = \
        df_last_execution['Fin de Ejecución'].max() - \
        df_first_execution['Tiempo de Llegada'].min()

    tiempo_total_actividad_cpu = \
        df_last_execution['Fin de Ejecución'].max() - \
        df_first_execution['Inicio de Ejecución'].min()
    ...
    metrics['rendimiento'] = len(df_first_execution) / tiempo_total_observado

    return metrics


# Función para calcular métricas de rendimiento
def calcular_metricas(df, algorithm, input_type, preemptive):
    if preemptive:
        return calcular_metricas_preemptive(df, algorithm, input_type)
    ...
    # hasta su finalización. Importante para entender el tiempo total que
    # un proceso requiere para completarse, incluyendo el tiempo de espera
    # y de ejecución.
    metrics['tiempo_retorno_promedio'] = df['Tiempo de Retorno'].mean()

    # Tiempo de espera promedio: Tiempo que cada proceso pasa en la cola
    # antes de su ejecución. Es una métrica crucial para evaluar la
    # percepción de rapidez de un sistema por parte del usuario.
    ...
    # espera para simplificación, pero en algoritmos con desalojo podría
    # ser diferente.
    metrics['tiempo_respuesta_promedio'] = df['Tiempo de Respuesta'].mean()

    # Variabilidad del tiempo de espera: Mide cuánto varía el tiempo de
    # espera entre diferentes procesos. Una alta variabilidad puede indicar
    # una falta de equidad en el tratamiento de procesos.
    metrics['var_tiempo_espera'] = df['Tiempo de Espera'].std()

    # Máximo tiempo de espera: El mayor tiempo que un proceso ha esperado
    # antes de ejecutarse. Útil para identificar casos extremos y evaluar
    # la equidad del algoritmo.
...
def main(file, output, algorithm, input_type):
    df = pd.read_csv(file)
    procesar_datos(df)
    preemptive = algorithm == 'rr' or algorithm == 'prioridad_preemptive'
